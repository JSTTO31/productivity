<template>
    <div>
        <h4 class="mt-5 mb-2">Tags</h4>
        <div class="d-flex flex-wrap">
            <div v-for="tag in selectedTags" :key="tag._id"  style="position: relative;">
                <v-chip variant="flat" size="large" class="text-capitalize mb-3 rounded mr-2" :color="tag.color">{{ tag.label }}</v-chip>
            ...
            <v-menu :close-on-content-click="false">
                <template #activator="{ props }">
                    <v-btn v-bind="props" class="text-capitalize mb-3" prepend-icon="mdi-plus" variant="outlined">Tag</v-btn>
                </template>
