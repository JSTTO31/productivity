<template>
    <v-list class="pa-2 bg-transparent" v-if="projects.length > 0">
        <project-list-item :index="n"  v-for="project, n in projects" :key="project.title"  :project="project"></project-list-item>
    </v-list>
...
await $project.getAll().then(() => {
    if(!route.params.project && projects.value.length > 0){
        router.push({name: 'r-access-projects-project', params: {project: projects.value[0]._id}})
    }
})
</script>
