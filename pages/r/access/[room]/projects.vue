<template>
    ...
    <v-app-bar class="d-flex align-center text-white border-b" density="compact"
        ...
        <div class="d-flex mr-6 ml-2">
            <v-card v-for="message, n in chats.messages.slice(7)" :key="message.sender" class="rounded-circle mr-n3">
    ...
    <v-main class="pr-0 h-screen" style="font-family: 'Roboto', sans-serif;overflow-y: auto;">
        <v-card color="transparent" class="h-100 rounded-0 pb-1" flat>
                ...
                <v-container class="px-0 h-100 mr-15 d-flex" fluid>
                    <v-col cols="3" xl="2" class="h-100 pa-0 mr-3 rounded-lg" v-for="section in sections"
                        ...
                        <v-card class=" px-5 py-3 rounded-lg"
                            ...
                            <project-list-card-item @click="$router.push({name: 'r-access-room-projects-task', params: {task: task.task_id}})" v-for="task in section.tasks"
                                :key="task.task_name" :task="task"></project-list-card-item>
            ...
            <v-dialog @dblclick:outside="$router.push('/')" contained persistent no-click-animation :model-value="$route.name == 'r-access-room-projects-task'" width="1000">
