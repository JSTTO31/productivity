<template>
    <v-main class="d-flex justify-center align-center bp-15">
        <v-card flat class="rounded-xl px-5 d-flex mt-5 mb-15 flex-column px-10" color="transparent" width="550" :disabled="loading">
            ...
            <v-form @submit.prevent="submit" class="mt-10">
                ...
                <div>
                    ...
                    <v-text-field v-model="$v.name.$model"
                        :error-messages="$v.name.$errors[0]?.$message as string || ''" id="name" variant="solo-filled"
                        flat class="my-2" density="comfortable" prepend-inner-icon="mdi-account" single-line
                ...
                <div>
                    ...
                    <v-text-field v-model="$v.email.$model"
                        ...
                        flat class="my-2 pr-0" density="comfortable" prepend-inner-icon="mdi-email-outline" single-line
                ...
                <div class="d-flex" style="gap: 10px;">
                    <div class="w-50">
                        ...
                        <v-text-field :type="togglePassword ? 'text' : 'password'" v-model="$v.password.$model"
                            ...
                            variant="solo-filled" flat class="my-2" density="comfortable" single-line
                            label="*******"></v-text-field>
                    </div>
                    <div class="w-50">
                        <label class="font-weight-bold" for="confirm">Confirmation</label>
                        <v-text-field :type="togglePassword ? 'text' : 'password'"
                            v-model="$v.password_confirmation.$model"
                            :error-messages="$v.password_confirmation.$errors[0]?.$message as string || ''" id="confirm"
                            variant="solo-filled" flat class="my-2" density="comfortable" single-line label="*******"
                ...
                <v-btn type="submit" class="rounded-lg mt-5 text-capitalize" append-icon="mdi-account" size="large"
                    color="success" :loading="loading">Create account</v-btn>
            </v-form>
        </v-card>
    </v-main>
</template>
